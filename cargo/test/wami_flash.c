    ...
    u16 (* const bayer)[WAMI_FLASH_IMG_NUM_COLS])
{
    ...
    for (row = PAD; row < WAMI_FLASH_IMG_NUM_ROWS-PAD; row += 2)
    {
        for (col = PAD; col < WAMI_FLASH_IMG_NUM_COLS-PAD; col += 2)
        {
            flashed[row-PAD][col-PAD].r = bayer[row][col];
        }
    }
    ...
    for (row = PAD; row < WAMI_FLASH_IMG_NUM_ROWS-PAD; row += 2)
    {
        for (col = PAD+1; col < WAMI_FLASH_IMG_NUM_COLS-PAD; col += 2)
        {
            flashed[row-PAD][col-PAD].g = bayer[row][col];
        }
    }

    /* Copy bottom-left green pixels through directly */
    for (row = PAD+1; row < WAMI_FLASH_IMG_NUM_ROWS-PAD; row += 2)
    {
        for (col = PAD; col < WAMI_FLASH_IMG_NUM_COLS-PAD; col += 2)
        {
            flashed[row-PAD][col-PAD].g = bayer[row][col];
    ...
    for (row = PAD+1; row < WAMI_FLASH_IMG_NUM_ROWS-PAD; row += 2)
    {
        for (col = PAD+1; col < WAMI_FLASH_IMG_NUM_COLS-PAD; col += 2)
        {
            flashed[row-PAD][col-PAD].b = bayer[row][col];
        }
    }
    ...
    for (row = PAD; row < WAMI_FLASH_IMG_NUM_ROWS-PAD; row += 2)
    {
        for (col = PAD; col < WAMI_FLASH_IMG_NUM_COLS-PAD; col += 2)
        {
            flashed[row-PAD][col-PAD].g = interp_G_at_RRR_or_G_at_BBB(
                bayer, row, col);
        }
    }
    ...
    for (row = PAD; row < WAMI_FLASH_IMG_NUM_ROWS-PAD; row += 2)
    {
        for (col = PAD+1; col < WAMI_FLASH_IMG_NUM_COLS-PAD; col += 2)
        {
            flashed[row-PAD][col-PAD].r = interp_R_at_GRB_or_B_at_GBR(
                bayer, row, col);
        }
    }
    ...
    for (row = PAD+1; row < WAMI_FLASH_IMG_NUM_ROWS-PAD; row += 2)
    {
        for (col = PAD; col < WAMI_FLASH_IMG_NUM_COLS-PAD; col += 2)
        {
            flashed[row-PAD][col-PAD].b = interp_R_at_GRB_or_B_at_GBR(
                bayer, row, col);
        }
    }
    ...
    for (row = PAD+1; row < WAMI_FLASH_IMG_NUM_ROWS-PAD; row += 2)
    {
        for (col = PAD; col < WAMI_FLASH_IMG_NUM_COLS-PAD; col += 2)
        {
            flashed[row-PAD][col-PAD].r = interp_R_at_GBR_or_B_at_GRB(
                bayer, row, col);
        }
    }
    ...
    for (row = PAD; row < WAMI_FLASH_IMG_NUM_ROWS-PAD; row += 2)
    {
        for (col = PAD+1; col < WAMI_FLASH_IMG_NUM_COLS-PAD; col += 2)
        {
            flashed[row-PAD][col-PAD].b = interp_R_at_GBR_or_B_at_GRB(
                bayer, row, col);
        }
    }
    ...
    for (row = PAD+1; row < WAMI_FLASH_IMG_NUM_ROWS-PAD; row += 2)
    {
        for (col = PAD+1; col < WAMI_FLASH_IMG_NUM_COLS-PAD; col += 2)
        {
            flashed[row-PAD][col-PAD].r = interp_R_at_BBB_or_B_at_RRR(
                bayer, row, col);
        }
    }
    ...
    for (row = PAD; row < WAMI_FLASH_IMG_NUM_ROWS-PAD; row += 2)
    {
        for (col = PAD; col < WAMI_FLASH_IMG_NUM_COLS-PAD; col += 2)
        {
            flashed[row-PAD][col-PAD].b = interp_R_at_BBB_or_B_at_RRR(
                bayer, row, col);
        }
    }
}
